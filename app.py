...
async def load_diar(model_name: str | None = None):
    ...
    async with _get_async_lock(_DIAR_LOAD_LOCKS, diar_model_name):
        ...
        def _make_diar_pipeline():
            ...
            # Do NOT pass cache_dir — env vars HUGGINGFACE_HUB_CACHE / HF_HOME
            # already point to the right location; overriding with TORCH_HOME
            # sends pyannote to the wrong path.
...
def _sweep():
    while True:
        ...
        gc.collect()
        # Use len() explicitly rather than truthiness to avoid any dict
        # subclass edge cases with `not cache`.
        with _POOLS_LOCK:
            pools_empty = len(WHISPER_POOLS) == 0
        a_empty = len(A_CACHE) == 0
        d_empty = len(D_CACHE) == 0
        logging.info(
            "[sweep] pools_empty=%s a_cache_keys=%s d_cache_keys=%s freeVRAM=%d MB",
            pools_empty, list(A_CACHE.keys()), list(D_CACHE.keys()), free_mb(),
        )
